...
def main():
    number = int(input("HOw many files will you be processing: "))
    for i in range(number):
        filename = tk_get_single_file()
        converter = MassHunterConverter(filename)
        converter.convert()
        converter.write(filename[:-4] + "_ID_FILE.csv")
